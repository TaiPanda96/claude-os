...
export function SessionSummary({ turns, gcEvents }: Props) {
  const stats = useMemo(() => {
    ...
    return deriveStats(
      points,
      firstEvent ? Math.round(firstEvent.ctx_pct_at_trigger * 1000) / 10 : null,
      firstEvent?.gc_type ?? null,
    );
  }, [turns, gcEvents]);

  const trendSymbol = stats.recentTrend === "rising" ? "↑" : stats.recentTrend === "declining" ? "↓" : "→";
  const trendColor = stats.recentTrend === "rising" ? GC_COLOR.clean : stats.recentTrend === "declining" ? GC_COLOR.hard_gc : "#aeaeb2";
  const deltaColor = stats.qualityDelta >= 0 ? GC_COLOR.clean : stats.qualityDelta < -0.2 ? GC_COLOR.hard_gc : GC_COLOR.soft_gc;

  const items: { label: string; value: string; color?: string; sub?: string }[] = [
    {
      label: "Peak quality",
      value: stats.peakQuality.toFixed(2),
      sub: `@ ${stats.peakCtxPct.toFixed(1)}% ctx`,
    },
    {
      label: "Inflection",
      value: stats.inflectionCtxPct != null ? `${stats.inflectionCtxPct.toFixed(1)}% ctx` : "none detected",
    ...
    },
    {
      ...
      value: stats.firstGCCtxPct != null ? `${stats.firstGCCtxPct.toFixed(1)}% ctx` : "none",
      color: stats.firstGCCtxPct != null
        ? (stats.firstGCType === "hard_gc" ? GC_COLOR.hard_gc : GC_COLOR.soft_gc)
        : "#48484a",
    ...
    <div style={styles.container}>
      {items.map((item) => (
        <div key={item.label} style={styles.stat}>
          ...
          <div style={{ ...styles.value, color: item.color ?? "#f2f2f7" }}>{item.value}</div>
