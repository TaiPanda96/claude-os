...
import { assembleMemorySliceForCompaction } from "./infrastructure/ai/assemble-memory-slice-for-compaction.js";
...
export async function compaction(
  ...
  try {
    ...
    for (const file of policy.memory_schema) {
      ...
      const slice = assembleMemorySliceForCompaction(
        turns,
        sessionId,
      ...
      let memoryOutputText: string;
      const prompt = buildMemoryCompactionPrompt(file, slice, existingContent, policy, {
        type: triggeredBy,
        detail: triggerDetail,
      });
      try {
        memoryOutputText = await summarizer.summarize(prompt, {
          merge: file.update_mode === "merge",
          maxTokens: OUTPUT_MAX_TOKENS,
        });
      } catch (err) {
        // If summarization fails for one file, skip writing it but continue with the
        // others; record the error in the event stream for observability.
      ...
      const result = await writeMemoryFileToDir(dir, file, memoryOutputText);
