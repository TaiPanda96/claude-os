...
if (!filePath || !existsSync(filePath)) {
  ...
  outer: for (const dir of readdirSync(PROJECTS)) {
    ...
    try {
      for (const file of readdirSync(dirPath).filter((f: string) => f.endsWith(".jsonl"))) {
        if (file.includes(sessionId ?? "")) { filePath = join(dirPath, file); break outer; }
      }
    } catch { continue; }
  }
}
...
// ── Minimal ingest (mirrors ingest.ts logic, single-file) ────────────────────
...
`).run({ $id: assistants[0].sessionId, $name: null, $model: model, $ctx: ctxWindow, $created: createdAt, $last: lastAt });
...
for (let i = 0; i < assistants.length; i++) {
  ...
  const latencyMs = parent ? new Date(turn.timestamp).getTime() - new Date(parent.timestamp).getTime() : 0;
  ...
  `).run({
    $id: turn.uuid, $sid: turn.sessionId, $idx: i,
    $in: u.input_tokens, $out: u.output_tokens,
    $cum: effectiveInput + u.output_tokens, $ctx: ctxPct,
    $lat: latencyMs, $stop: turn.message.stop_reason,
    ...
    $cr: cacheRead, $cc: cacheCreation, $eff: effectiveInput,
    $cwd: turn.cwd ?? "",
  });
  ...
  if ((result as { changes: number }).changes > 0) {
    ...
    if (gcState !== lastGCState && gcState !== "clean") {
      db.prepare(`INSERT OR IGNORE INTO gc_events (id, session_id, gc_type, ctx_pct_at_trigger, created_at) VALUES ($id, $sid, $gc, $ctx, $ts)`)
        .run({ $id: `${turn.uuid}-gc`, $sid: turn.sessionId, $gc: gcState, $ctx: ctxPct, $ts: new Date(turn.timestamp).getTime() });
    }
    lastGCState = gcState;
  }
